...
contract BABYTOKEN is ERC20, Ownable {
    ...
    uint256 public buyLafiteFee;
    uint256 public sellLafiteFee;
    ...
    constructor(
        ...
        buyLafiteFee = buyFeeSetting_[2];
        buyDeadFee = buyFeeSetting_[3];

        sellTokenRewardsFee = sellFeeSetting_[0];
        sellLiquidityFee = sellFeeSetting_[1];
        sellLafiteFee = sellFeeSetting_[2];
        sellDeadFee = sellFeeSetting_[3];

        require(buyTokenRewardsFee.add(buyLiquidityFee).add(buyLafiteFee).add(buyDeadFee) <= 25, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee).add(sellLafiteFee).add(sellDeadFee) <= 25, "Total sell fee is over 25%");
        ...
        excludeFromFees(0x175A29FacB5F2A8bec44Be95feD7E907c6d7f2e1, true);

        _mint(0x175A29FacB5F2A8bec44Be95feD7E907c6d7f2e1, totalSupply);
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                MFee = amount.mul(buyLafiteFee).div(100);
            ...
            if(automatedMarketMakerPairs[to]){
                ...
                AmountTokenRewardsFee += RFee;
                MFee = amount.mul(sellLafiteFee).div(100);
                AmountMarketingFee += MFee;
